 <div class="col-md-3 col-lg-2 px-0 sidebar" id="sidebar">
     ...
     <nav class="nav flex-column">
         ...
         <a href="{{ route('user.security') }}"
             class="nav-link {{ request()->routeIs('user.security') ? 'active' : '' }}">
         ...
         <a href="{{ route('user.notifications') }}"
             ...
             <i class="fas fa-bell {{ auth()->user()->customerNotifications()->whereNull('read_at')->count() > 0 ? 'text-warning' : '' }}"></i>
             Notifications
             @if (auth()->user()->customerNotifications()->whereNull('read_at')->count() > 0)
                 <sup class="bg-warning text-white rounded shadow p-1">
